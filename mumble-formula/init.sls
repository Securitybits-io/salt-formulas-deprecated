#Package.mumble-server init.sls
{% set mumble = pillar['mumble'] %}
...
configure mumble-server:
  file.managed:
    ...
    - contents: |
        ...
        welcometext={{ mumble['motd'] | default('Hello from SecurityMumble') }}

        ; Port to bind TCP and UDP sockets to.
        port={{ mumble['port'] | default(64738) }}
        ...
        serverpassword= {{ mumble['serverpassword'] }}
        ...
        ;
        registerName={{ mumble['rootchannelname'] | default("Battalion Network") }}
...
{% if slack["post"] == True %}
post superuser password to slack:
  slack.post_message:
    - channel: {{ slack["channel-name"] }}
    - from_name: {{ grains['host'] }}
    - message: __slot__:salt:cmd.run("grep Password '/var/log/mumble-server/mumble-server.log'")
